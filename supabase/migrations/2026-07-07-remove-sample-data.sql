...
--
-- Safe to run on any project: tables that don't exist yet (e.g. tasks,
-- activities, deal_activities from newer migrations) are skipped.

do $$
begin
  -- Everything hanging off the 10 sample contacts.
  if to_regclass('public.activities') is not null then
    delete from public.activities where contact_id in (
  ...
  -- The 10 sample contacts themselves.
  ...
  if to_regclass('public.deals') is not null then
    if to_regclass('public.deal_activities') is not null then
